...
## 更新摘要
**变更内容**
- 新增思维摘要功能，支持大脑暂停/完成状态下的总结展示
- 增强 tool_gap 认知元素类型的可视化支持
- 更新 BrainView 组件以支持新的思维摘要和工具缺口可视化
- 新增 ThinkingSummary 类型定义和 API 方法
...
**更新** 路由从 `/project/:id` 更正为 `/brain/:brainId`，确保路由一致性
...
H[思维摘要卡片<br/>ThinkingSummaryCard]
I[工具缺口可视化<br/>tool_gap Visualization]
end
subgraph "业务逻辑层"
J[api.ts<br/>REST API 客户端]
K[类型定义<br/>types.ts]
L[认证守卫<br/>RequireAuth]
M[思维摘要服务<br/>brain_summary.py]
N[认知元素服务<br/>cognitive.py]
end
subgraph "路由层"
O[App.tsx<br/>路由配置]
P[main.tsx<br/>路由包装器]
...
subgraph "数据层"
Q[后端 API<br/>/ainstein/api]
R[本地存储<br/>localStorage]
S[观察员日志<br/>observer_logs]
T[思维摘要缓存<br/>thinking_summary]
end
A --> J
B --> J
C --> J
...
BrainList->>Router : navigate('/brain/ : brainId')
...
CreateBrain->>Router : navigate('/brain/ : brainId')
```
...
**更新** BrainView 页面现已包含增强的思维摘要功能和 tool_gap 认知元素类型可视化支持
...
+useState error : string
+useState thinkingSummary : ThinkingSummary | null
+useState summaryExpanded : boolean
+useEffect(load) void
+useEffect(render) void
+useEffect(highlight) void
+useEffect(zoom) void
+useEffect(summary) void
}
...
6. **思维摘要展示**：在大脑暂停或完成状态下显示总结卡片
7. **工具缺口可视化**：支持 tool_gap 类型的认知元素可视化

#### 增强功能详解

**思维摘要功能**：
- 当大脑状态为 `paused` 或 `completed` 时自动加载思维摘要
- 提供核心答案、关键洞察、被否定的假说、开放问题和方法论突破的展示
- 支持展开/收起详细内容

**tool_gap 可视化支持**：
- 新增 `tool_gap` 认知元素类型的颜色映射（紫色调）
- 支持工具缺口的标签显示和关系可视化
- 在 CE 类型筛选中提供专门的工具缺口过滤
...
User->>D3 : 拖拽节点/缩放
D3->>BrainView : 更新图形布局
BrainView->>API : getThinkingSummary() - 加载思维摘要
API-->>BrainView : 思维摘要数据
BrainView->>User : 显示思维摘要卡片
```
...
**章节来源**
- [BrainView.tsx:1-1273](file://frontend/src/pages/BrainView.tsx#L1-L1273)

### 项目详情页面

**更新** 项目详情页面现已更名为 `ProjectDetail`，但路由仍保持 `/project/:id` 兼容性
...
+getThinkingSummary(id) Promise
+generateObserverSummary(id, data) Promise
+getObserverLogs(id, params) Promise
+getLatestObserverLog(id) Promise
}
class RequestHelper {
+request(path, opts) Promise
...
**章节来源**
- [api.ts:1-223](file://frontend/src/api.ts#L1-L223)
...
THINKING_SUMMARY {
string core_answer
THINKING_SUMMARY_INSIGHT[] key_insights
THINKING_SUMMARY_REFUTED[] refuted
string[] open_questions
string methodology_note
string generated_at
}
THINKING_SUMMARY_INSIGHT {
string summary
number ce_id
number confidence
}
THINKING_SUMMARY_REFUTED {
string claim
number ce_id
}
...
THINKING_SUMMARY ||--o{ THINKING_SUMMARY_INSIGHT : "contains"
THINKING_SUMMARY ||--o{ THINKING_SUMMARY_REFUTED : "contains"
```

**图表来源**
- [types.ts:1-220](file://frontend/src/types.ts#L1-L220)
...
5. **工具缺口过滤**：CE 类型筛选功能支持快速过滤 tool_gap 类型
...
**解决步骤**：
1. 检查后端 `brain_summary.py` 服务状态
2. 确认大脑状态为 `paused` 或 `completed`
3. 清除缓存或强制重新生成
...
**解决步骤**：
1. 检查 `cognitive.py` 中的 CE_TYPES 定义
2. 验证 D3.js 颜色映射配置
3. 确认工具缺口数据正确加载
...
7. **智能总结**：新增的思维摘要功能提供大脑思考成果的结构化展示
8. **认知增强**：支持 tool_gap 等高级认知元素的可视化分析

该应用为 AI 深度研究平台提供了完整且易用的前端界面，支持复杂的大脑管理和研究工作流需求。新增的思维摘要功能和工具缺口可视化进一步增强了应用的认知分析能力，为用户提供了更深入的大脑运作洞察。
...
5. **思维摘要功能**：大脑暂停/完成状态下的智能总结展示
6. **工具缺口可视化**：支持 tool_gap 认知元素的专门可视化
7. **路由一致性改进**：从 `/project/:id` 更正为 `/brain/:brainId`，确保路由命名一致性
8. **保留兼容路由**：旧版 `/dashboard` 和 `/project/:id` 路由用于向后兼容
9. **增强的类型系统**：新增 ThinkingSummary 类型定义支持智能总结功能

这些变更使应用从传统的项目管理转向了更先进的人工智能大脑管理平台，显著提升了认知分析和思维洞察的能力。