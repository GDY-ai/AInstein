...
**更新** 新增思维收敛策略机制，包括收敛压力调度、强制综合脉冲、问题深度控制和双轨终止等新功能，有效防止大脑陷入无限发散的"精致化陷阱"。
...
Bus --> DB
Bus --> Orchestrator
Orchestrator --> AgentSci
Orchestrator --> AgentDir
Orchestrator --> AgentRes
```
...
- [orchestrator.py:1-1636](file://orchestrator.py#L1-L1636)
...
## 架构总览
系统采用「三层代理 + 三轮引擎 + 认知元素双写 + 事件总线 + 思维收敛编排器」的架构。后端通过 Flask 提供 REST API，前端通过 React 渲染界面。Agent 代理通过 LLM 生成策略与主题，引擎执行研究流程并将结果写入传统表与认知元素表，事件总线负责跨组件通信，思维收敛编排器确保大脑在适当的时机从发散转向收敛。
...
### 思维收敛策略机制
**新增** 系统实现了完整的思维收敛策略，有效防止大脑陷入无限发散的"精致化陷阱"：

#### 收敛压力调度
- **监控窗口**：检查最近 20 个认知元素的类型分布
- **探索类型**：observation, question, hypothesis, evidence, counter_evidence
- **收敛类型**：inference, argument, conclusion, consensus, insight
- **阈值判断**：当探索/收敛比 > 5:1 时，进入收敛模式
- **收敛模式**：优先派遣 reasoner/synthesizer/critic，暂停 explorer

#### 强制综合脉冲
- **触发条件**：每产出 20 个认知元素（自上次综合以来）
- **作用机制**：强制插入一次 synthesizer 综合轮
- **区别于终止**：不会导致大脑停止，仅打断发散模式

#### 问题深度控制
- **开放问题监控**：当 open 状态的 question 占总 CE 的 30% 以上时
- **拒绝新问题**：阻止继续提问，迫使大脑回答已有问题
- **转向整合**：避免问题链无限延伸

#### 双轨终止策略
- **主轨**：synthesizer 产出 conclusion 且置信度 >= 0.75 → 自动完成
- **兜底轨**：CE >= 500 或运行 >= 1小时 → 强制触发最终总结（一次性）

```mermaid
flowchart TD
Start(["开始思考循环"]) --> CheckPressure["检查收敛压力"]
CheckPressure --> Ratio{"探索/收敛比 > 5:1?"}
Ratio --> |是| ConvergeMode["进入收敛模式"]
Ratio --> |否| ExploreMode["保持探索模式"]
ConvergeMode --> ForceSynth["强制综合脉冲(每20CE)"]
ForceSynth --> SynthCheck{"synthesizer 置信度 >= 0.75?"}
SynthCheck --> |是| AutoStop["自动停止"]
SynthCheck --> |否| Continue["继续思考"]
ExploreMode --> QuestionDepth["检查问题深度"]
QuestionDepth --> OpenQ{"open question 占比 > 30%?"}
OpenQ --> |是| ForceConverge["强制收敛"]
OpenQ --> |否| Continue
ForceConverge --> ConvergeMode
Continue --> Fallback{"CE>=500 或 运行>=1小时?"}
Fallback --> |是| FinalSummary["最终总结"]
Fallback --> |否| End(["结束循环"])
AutoStop --> End
FinalSummary --> End
```
...
**图表来源**
- [orchestrator.py:989-1120](file://orchestrator.py#L989-L1120)
- [orchestrator.py:1121-1190](file://orchestrator.py#L1121-L1190)
- [orchestrator.py:1191-1315](file://orchestrator.py#L1191-L1315)
- [orchestrator.py:1316-1395](file://orchestrator.py#L1316-L1395)

**章节来源**
- [orchestrator.py:88-130](file://orchestrator.py#L88-L130)
- [orchestrator.py:989-1120](file://orchestrator.py#L989-L1120)
- [orchestrator.py:1121-1190](file://orchestrator.py#L1121-L1190)
- [orchestrator.py:1191-1315](file://orchestrator.py#L1191-L1315)
- [orchestrator.py:1316-1395](file://orchestrator.py#L1316-L1395)
- [README.md:256-294](file://README.md#L256-L294)
...
- **思维收敛编排器**：与事件总线、认知元素、Agent 框架深度集成，实现智能调度。
...
- **思维收敛编排器**：收敛压力检查、强制综合脉冲、问题深度控制均采用轻量级查询，避免对主循环造成性能负担。
...
- **思维收敛编排器异常**：检查收敛压力参数配置，确认数据库连接正常，查看编排器日志中的收敛压力检查失败信息。
...
## 结论
本项目通过「三级代理 + 三轮引擎 + 认知元素双写 + 事件总线 + 思维收敛编排器」构建了从「种子问题」到「知识图谱」的完整闭环。系统在 v1 阶段已实现基本研究流程与知识沉淀，新增的思维收敛策略机制有效防止大脑陷入无限发散，确保研究过程的效率与质量。后续将推进到「硅基大脑」形态，实现去层级化、ATA 协议、博弈与共识机制、力导向可视化与用户系统，逐步实现真正的「思考」涌现。

## 附录
- 快速开始：克隆仓库、安装依赖、配置环境变量、初始化数据库、启动后端与前端。
- 部署：使用 Gunicorn 启动，结合 Nginx 反代与前端构建产物。
- 蓝图与路线图：参见 README 中的愿景路线图与核心概念速览。
- **思维收敛策略参数**：当前参数均为硬编码常量，后续将迁移到 `brains` 表的 `config_json` 字段，支持每个大脑独立配置。
